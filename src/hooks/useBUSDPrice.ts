...
import { useCaratBuyAmount } from 'hooks/useCaratTrade'
...
export function useCaratPrice() {
  const caratPrice = useCaratBuyAmount(new TokenAmount(tokens.carat, JSBI.BigInt(DEFAULT_TOKEN_DECIMAL)), tokens.cake)
  const scadsAmount = tryParseAmount('1', tokens.cake)
  const scadsPrice = useTradeExactIn(scadsAmount, tokens.busd)

  const ethsnacksPrice = useCaratBuyAmount(
    new TokenAmount(tokens.ethsnacks, JSBI.BigInt(DEFAULT_TOKEN_DECIMAL)),
    tokens.weth,
  )
  const ethusdPrice = useEthUSDPrice()

  const btcsnacksPrice = useCaratBuyAmount(
  ...
  return {
    caratPrice: parseFloat(caratPrice?.toExact() ?? '0') * parseFloat(scadsPrice?.outputAmount?.toExact() ?? '0'),
